...
const Home = () => {
	...
	const addDemoMedicines = async () => {
		...
		try {
			const promises = drugs.map(async (item) => {
				const newDrug = {
					name: item,
				};
				await (window as any).beeclinicAPI.addMedicine(newDrug);
			});
			await Promise.all(promises);
			console.log('Đã thêm dữ liệu thuốc mẫu vào local');
		} catch (error) {
			console.log(error);
		}
	};
				...
				<div className='container'>
					<div className='row mt-5'>
						<div className='col-8 offset-2'>
							...
							<Button
								onClick={addDemoMedicines}
								type='link'
								className='mb-2 p-0'
								size='small'>
								Thêm dữ liệu thuốc mẫu
							</Button>
							<div className='my-3'>
								<AutoComplete
									notFoundContent={
